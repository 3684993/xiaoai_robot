...
class InputController:
    ...
    def __init__(self, x_step_size=1.0, y_step_size=1.0, z_step_size=1.0, wrist_step_size=1.0):
        """
        ...
        Args:
            ...
            wrist_step_size: Base movement step size for wrist in meters
        """
        self.x_step_size = x_step_size
        self.y_step_size = y_step_size
        self.z_step_size = z_step_size
        self.wrist_step_size = wrist_step_size
        self.running = True
    ...
    def get_deltas(self):
        """Get the current movement deltas (dx, dy, dz, dwrist) in meters."""
...
class KeyboardController(InputController):
    ...
    def __init__(self, x_step_size=1.0, y_step_size=1.0, z_step_size=1.0, wrist_step_size=1.0):
        super().__init__(x_step_size, y_step_size, z_step_size, wrist_step_size)
        self.key_states = {
            "forward_x": False,
            "backward_x": False,
            "forward_y": False,
    ...
    def start(self):
        ...
        def on_press(key):
            try:
                if key == keyboard.Key.up:
                    ...
                    self.key_states["forward_wrist"] = True
                elif key == keyboard.Key.ctrl_r:
                    self.key_states["backward_wrist"] = True
        ...
        def on_release(key):
            try:
                if key == keyboard.Key.up:
                    ...
                    self.key_states["forward_wrist"] = False
                elif key == keyboard.Key.ctrl_r:
                    self.key_states["backward_wrist"] = False
                elif key == keyboard.Key.enter:
        ...
        print("  Ctrl_L and Ctrl_R: Move wrist")
    ...
    def get_deltas(self):
        ...
        delta_x = delta_y = delta_z = delta_wrist = 0.0
        ...
        if self.key_states["backward_z"]:
            delta_z -= self.z_step_size
        if self.key_states["forward_wrist"]:
            delta_wrist += self.wrist_step_size
        if self.key_states["backward_wrist"]:
            delta_wrist -= self.wrist_step_size

        return delta_x, delta_y, delta_z, delta_wrist

class GamepadController(InputController):
    """Generate motion deltas from gamepad input."""

    def __init__(self, x_step_size=1.0, y_step_size=1.0, z_step_size=1.0, wrist_step_size=1.0, deadzone=0.1):
        super().__init__(x_step_size, y_step_size, z_step_size, wrist_step_size)
    ...
    def start(self):
        ...
        print("  Right analog stick: Move in Z and wrist axes")
    ...
    def get_deltas(self):
        ...
        try:
            ...
            y_input = self.joystick.get_axis(0)  # 左摇杆左右
            x_input = self.joystick.get_axis(1)  # 左摇杆上下

            # Right stick X and Y (typically axes 4 and 3)
            wrist_input = self.joystick.get_axis(3)  # 右摇杆左右
            z_input = self.joystick.get_axis(4)      # 右摇杆上下

            # Apply deadzone to avoid drift
            x_input = 0 if abs(x_input) < self.deadzone else x_input
            y_input = 0 if abs(y_input) < self.deadzone else y_input
            z_input = 0 if abs(z_input) < self.deadzone else z_input
            wrist_input = 0 if abs(wrist_input) < self.deadzone else wrist_input
                        # 非线性响应曲线（平方曲线，使控制更精细）
            def apply_curve(value, exponent=2.0):
                sign = 1 if value >= 0 else -1
                return sign * (abs(value) ** exponent)

            x_input = apply_curve(x_input)
            y_input = apply_curve(y_input)
            z_input = apply_curve(z_input)
            wrist_input = apply_curve(wrist_input)

            # Calculate deltas (note: may need to invert axes depending on controller)
            delta_x = -x_input * self.x_step_size      # 左摇杆上下
            delta_y = y_input * self.y_step_size      # 左摇杆左右
            delta_z = z_input * self.z_step_size      # 右摇杆左右
            delta_wrist = -wrist_input * self.wrist_step_size  # 右摇杆上下
...
class GamepadControllerHID(InputController):
    ...
    def start(self):
        ...
        try:
            ...
            logging.info("  Right analog stick: Move in Z and wrist axes")
    ...
    def get_deltas(self):
        ...
        delta_wrist = -self.right_x * self.wrist_step_size  # Wrist movement
